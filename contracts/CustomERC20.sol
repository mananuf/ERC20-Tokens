...
contract CustomERC20 {
    ...
    // External function to test _mint
    function testMint(address account, uint256 amount) external {
        _mint(account, amount);
    }

    function testInternalTransfer(address to, uint256 amount) external {
        transfer(to, amount);
    }

    // External function to test _burn
    function testBurn(address account, uint256 amount) external {
        _burn(account, amount);
    }

    // External function to spend allowance
    function testSpendAllowance(address owner, address spender, uint256 amount) external {
        _spendAllowance(owner, spender, amount);
    }

    function test_approve(address owner, address spender, uint256 amount) external {
        _approve(owner, spender, amount);
    }

    function test_transfer(address from, address to, uint256 amount) external {
        _transfer(from, to, amount);
    }
}
